...
export default function TeacherFormModal({ open, onClose, password, onSubmit, initialData, saving }) {
  const emptyForm = { name: "", phone: "", password: `${password}`, role: "Teacher" };
  ...
  useEffect(() => {
    if (open) {
      setForm(
        initialData
          ? {
            name: initialData.name, phone: initialData.phone, role: "Teacher"
          }
          : emptyForm
      );
      setErrors({});
  ...
  return (
    <Modal
      open={open}
      onClose={onClose}
      title={isEdit ? t.teachers.editTeacher : t.teachers.newTeacher}
      footer={
        <>
    ...
    >
      <div className="space-y-4">
        ...
        <div className={`grid ${!initialData && 'md:grid-cols-2'} grid-col-1 gap-4`}>
          ...
          {
            !initialData && (
              <Field label={t.teachers.password} >
                <TextInput value={password} disabled />
              </Field>
            )
          }
        </div>
